...
module LoadPipe(
  ...
  wire             resp_bits_handled =
    s2_miss_req_valid_dup & io_miss_req_ready & ~io_mq_enq_cancel & ~io_wbq_block_miss_req
    & io_miss_resp_handled;
  wire             _GEN_1 = io_bank_conflict_slow & s2_valid;
  wire             _GEN_2 = s2_valid & (|s2_hit_prefetch) & (|s2_real_way_en);
  wire             _GEN_3 = _GEN_2 & ~s2_hit_access;
  wire             _useful_prefetch_T = s2_req_instrtype == 4'h3;
  wire             _io_prefetch_info_fdp_demand_miss_T = s2_req_instrtype != 4'h3;
  wire             io_prefetch_info_fdp_demand_miss_0 =
    s2_valid & _io_prefetch_info_fdp_demand_miss_T & ~_s2_hit_T_2 & s2_req_isFirstIssue;
  reg              REG_4;
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset & ~REG_1) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: LoadPipe only accepts load req / softprefetch read or write!\n    at LoadPipe.scala:146 assert(RegNext(!(s0_valid && (s0_req.cmd =/= MemoryOpConstants.M_XRD && s0_req.cmd =/= MemoryOpConstants.M_PFR && s0_req.cmd =/= MemoryOpConstants.M_PFW))), \"LoadPipe only accepts load req / softprefetch read or write!\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & ~REG_2) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: tag should not match with more than 1 way\n    at LoadPipe.scala:274 assert(RegNext(!s1_valid || PopCount(s1_tag_match_way_dup_dc) <= 1.U), \"tag should not match with more than 1 way\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & ~REG_4) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: load pipeline should never be blocked\n    at LoadPipe.scala:515 assert(RegNext(s1_ready && s2_ready), \"load pipeline should never be blocked\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  ...
  wire             _GEN_4 = s3_clear_pf_flag_en_probe & ~io_counter_filter_query_resp;
  wire             _GEN_5 = s1_valid & io_lsu_s1_kill;
  wire             _GEN_6 = s1_valid & s1_tag_match_dup_dc;
  wire             _GEN_7 = s2_valid & resp_bits_replay;
  wire             _GEN_8 = _GEN_7 & s2_nack_data;
  wire             _GEN_9 = _GEN_7 & s2_nack_no_mshr;
  wire             _GEN_10 = _GEN_7 & io_bank_conflict_slow;
  wire             _GEN_11 = s2_valid & (|s2_real_way_en);
  wire             _GEN_12 = s2_valid & ~(|s2_real_way_en);
  wire             _GEN_13;
  assign _GEN_13 = _GEN_12;
  wire             _GEN_14;
  assign _GEN_14 = _GEN_12;
  wire             _GEN_15 = s2_valid & (|s2_real_way_en) & ~resp_bits_replay;
  wire             _GEN_16 = _GEN_6 & ~io_disable_ld_fast_wakeup;
  wire             _GEN_17 =
  ...
  wire             s1_will_send_miss_req =
    ...
    & ~(s1_tag_match_dup_dc
        ...
        & _s1_hit_meta_T_8 == _GEN_18[_s1_new_hit_coh_r_T]);
  always @(posedge clock) begin
    REG_1 <=
    ...
    REG_2 <=
      ~s1_valid
      | 3'({1'h0,
            2'({1'h0, _s1_tag_match_way_dup_dc_T_3}
               + {1'h0, _s1_tag_match_way_dup_dc_T_7})}
           + {1'h0,
              2'({1'h0, _s1_tag_match_way_dup_dc_T_11}
                 + {1'h0, _s1_tag_match_way_dup_dc_T_15})}) < 3'h2;
    if (s1_valid) begin
      ...
      s2_hit_access <=
        _s1_tag_match_way_dup_dc_T_3 & io_extra_meta_resp_0_access
        | _s1_tag_match_way_dup_dc_T_7 & io_extra_meta_resp_1_access
        | _s1_tag_match_way_dup_dc_T_11 & io_extra_meta_resp_2_access
        | _s1_tag_match_way_dup_dc_T_15 & io_extra_meta_resp_3_access;
    end
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        REG_1 = _RANDOM[5'h0][1];
        s1_valid = _RANDOM[5'h0][2];
        s1_req_cmd = _RANDOM[5'h0][7:3];
        s1_req_vaddr = {_RANDOM[5'h0][31:8], _RANDOM[5'h1][25:0]};
        s1_req_instrtype = _RANDOM[5'h6][19:16];
        s1_req_isFirstIssue = _RANDOM[5'h6][20];
        s1_req_lqIdx_flag = _RANDOM[5'h6][26];
        s1_req_lqIdx_value = {_RANDOM[5'h6][31:27], _RANDOM[5'h7][1:0]};
        s1_load128Req = _RANDOM[5'h7][10];
        s1_bank_oh = _RANDOM[5'h7][19:11];
        REG_2 = _RANDOM[5'h7][26];
        s2_valid = _RANDOM[5'h7][27];
        s2_valid_dup = _RANDOM[5'h7][28];
        s2_req_cmd = {_RANDOM[5'h7][31:29], _RANDOM[5'h8][1:0]};
        s2_req_instrtype = _RANDOM[5'hE][13:10];
        s2_req_isFirstIssue = _RANDOM[5'hE][14];
        s2_load128Req = _RANDOM[5'hF][4];
        s2_paddr = {_RANDOM[5'hF][31:5], _RANDOM[5'h10][20:0]};
        s2_vaddr = {_RANDOM[5'h10][31:21], _RANDOM[5'h11], _RANDOM[5'h12][6:0]};
        s2_real_way_en = _RANDOM[5'h12][29:26];
        s2_tag_errors = _RANDOM[5'h13][8:5];
        s2_tag_match_way = _RANDOM[5'h13][12:9];
        s2_tag_match = _RANDOM[5'h13][13];
        s2_can_send_miss_req = _RANDOM[5'h13][14];
        s2_can_send_miss_req_dup = _RANDOM[5'h13][15];
        s2_hit_coh_state = _RANDOM[5'h13][20:19];
        s2_nack_data = _RANDOM[5'h13][22];
        s2_flag_error = _RANDOM[5'h13][23];
        s2_hit_prefetch = _RANDOM[5'h13][26:24];
        s2_hit_access = _RANDOM[5'h13][27];
        io_miss_req_bits_pf_source_REG = _RANDOM[5'h13][30:28];
        io_miss_req_bits_pf_source_REG_1 = {_RANDOM[5'h13][31], _RANDOM[5'h14][1:0]};
        REG_4 = _RANDOM[5'h14][3];
        s3_valid = _RANDOM[5'h14][4];
        s3_load128Req = _RANDOM[5'h14][5];
        s3_vaddr = {_RANDOM[5'h14][31:6], _RANDOM[5'h15][23:0]};
        s3_paddr = {_RANDOM[5'h15][31:24], _RANDOM[5'h16], _RANDOM[5'h17][7:0]};
        s3_hit = _RANDOM[5'h17][8];
        s3_tag_match_way = _RANDOM[5'h17][12:9];
        s3_req_instrtype = _RANDOM[5'h17][16:13];
        s3_banked_data_resp_word =
          {_RANDOM[5'h17][31:17],
           ...
           _RANDOM[5'h1A],
           _RANDOM[5'h1B][16:0]};
        s3_tag_error = _RANDOM[5'h1B][17];
        s3_flag_error = _RANDOM[5'h1B][18];
        s3_hit_prefetch = _RANDOM[5'h1B][21:19];
        io_replace_access_bits_set_REG = _RANDOM[5'h1B][29:22];
        io_replace_access_bits_set_REG_1 = {_RANDOM[5'h1B][31:30], _RANDOM[5'h1C][5:0]};
        io_replace_access_bits_way_REG = _RANDOM[5'h1C][7:6];
        io_replace_access_bits_way_REG_1 = _RANDOM[5'h1C][9:8];
        io_perf_0_value_REG = _RANDOM[5'h1C][10];
        io_perf_0_value_REG_1 = _RANDOM[5'h1C][11];
        io_perf_1_value_REG = _RANDOM[5'h1C][12];
        io_perf_1_value_REG_1 = _RANDOM[5'h1C][13];
        io_perf_2_value_REG = _RANDOM[5'h1C][14];
        io_perf_2_value_REG_1 = _RANDOM[5'h1C][15];
        io_perf_3_value_REG = _RANDOM[5'h1C][16];
        io_perf_3_value_REG_1 = _RANDOM[5'h1C][17];
        io_perf_4_value_REG = _RANDOM[5'h1C][18];
        io_perf_4_value_REG_1 = _RANDOM[5'h1C][19];
      `endif // RANDOMIZE_REG_INIT
      if (reset) begin
        s1_valid = 1'h0;
        s2_valid = 1'h0;
  ...
  assign io_lsu_resp_bits_handled = resp_bits_handled;
  assign io_lsu_s2_bank_conflict = io_bank_conflict_slow;
  assign io_lsu_s2_mq_nack =
    ~(|s2_real_way_en) & (s2_nack_no_mshr | io_mq_enq_cancel | io_wbq_block_miss_req);
  ...
  assign io_prefetch_info_naive_late_prefetch_hit =
    _late_prefetch_hit_T & _useful_prefetch_T & (|(s2_hit_prefetch[2:1]));
  assign io_prefetch_info_naive_late_load_hit =
    _late_prefetch_hit_T & _useful_prefetch_T & ~(|(s2_hit_prefetch[2:1]));
  assign io_prefetch_info_naive_useful_prefetch =
    s2_valid & _useful_prefetch_T & resp_bits_handled & ~io_miss_resp_merged;
  assign io_prefetch_info_naive_prefetch_hit =
    s2_valid & _io_prefetch_info_fdp_demand_miss_T & _s2_hit_T_2
    & (|(s2_hit_prefetch[2:1])) & s2_req_isFirstIssue;
  assign io_prefetch_info_fdp_useful_prefetch =
    s3_clear_pf_flag_en_probe & ~io_counter_filter_query_resp;
  assign io_prefetch_info_fdp_demand_miss = io_prefetch_info_fdp_demand_miss_0;
  assign io_prefetch_info_fdp_pollution =
    io_prefetch_info_fdp_demand_miss_0 & io_bloom_filter_query_resp_valid
