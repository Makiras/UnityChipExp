...
module MSHR(
  ...
  output [63:0] acquire_period,
  output [63:0] probe_period,
  output [63:0] release_period
);
    ...
    always @(posedge clock) begin
      if (~reset & req_valid & req_prefetch & dirResult_hit) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: MSHR can not receive prefetch hit req\n    at MSHR.scala:112 assert(!(req_valid && req_prefetch && dirResult.hit), \"MSHR can not receive prefetch hit req\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & io_resps_sink_c_valid & ~io_status_bits_w_c_resp_0) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at MSHR.scala:575 assert(!(c_resp.valid && !io.status.bits.w_c_resp))\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & io_resps_sink_d_valid
          & ~(~state_w_grantlast | ~state_w_grant | ~state_w_releaseack)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at MSHR.scala:576 assert(!(d_resp.valid && !io.status.bits.w_d_resp))\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  ...
  reg              probe_ts;
  reg              release_ts;
  wire             _acquire_ts_T = io_tasks_source_a_ready & ~state_s_acquire;
  ...
  wire             _GEN_13 = will_free & req_valid;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (_GEN_13)
        ...
        timer <= 64'h1;
      req_valid <= ~_GEN_13 & (io_alloc_valid | req_valid);
      ...
      if (_probe_ts_T)
        probe_ts <= timer[0];
      if (~mp_grant_valid & mp_release_valid & io_tasks_mainpipe_ready)
        release_ts <= timer[0];
    end
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        timer = {_RANDOM[5'h0][31:7], _RANDOM[5'h1], _RANDOM[5'h2][6:0]};
        ...
        req_reqSource = _RANDOM[5'h7][16:13];
        dirResult_hit = _RANDOM[5'hA][12];
        dirResult_tag = {_RANDOM[5'hA][31:13], _RANDOM[5'hB][11:0]};
        dirResult_set = _RANDOM[5'hB][20:12];
        dirResult_way = _RANDOM[5'hB][23:21];
        dirResult_meta_dirty = _RANDOM[5'hB][24];
        dirResult_meta_state = _RANDOM[5'hB][26:25];
        dirResult_meta_clients = _RANDOM[5'hB][27];
        dirResult_meta_alias = _RANDOM[5'hB][29:28];
        dirResult_meta_prefetch = _RANDOM[5'hB][30];
        dirResult_meta_accessed = _RANDOM[5'hC][2];
        state_s_acquire = _RANDOM[5'hC][17];
        state_s_rprobe = _RANDOM[5'hC][18];
        state_s_pprobe = _RANDOM[5'hC][19];
        state_s_release = _RANDOM[5'hC][20];
        state_s_probeack = _RANDOM[5'hC][21];
        state_s_refill = _RANDOM[5'hC][22];
        state_s_retry = _RANDOM[5'hC][23];
        state_s_cmoresp = _RANDOM[5'hC][24];
        state_w_rprobeackfirst = _RANDOM[5'hC][25];
        state_w_rprobeacklast = _RANDOM[5'hC][26];
        state_w_pprobeackfirst = _RANDOM[5'hC][27];
        state_w_pprobeacklast = _RANDOM[5'hC][28];
        state_w_pprobeack = _RANDOM[5'hC][29];
        state_w_grantfirst = _RANDOM[5'hC][30];
        state_w_grantlast = _RANDOM[5'hC][31];
        state_w_grant = _RANDOM[5'hD][0];
        state_w_releaseack = _RANDOM[5'hD][1];
        state_w_replResp = _RANDOM[5'hD][2];
        mergeA = _RANDOM[5'hD][3];
        mp_grant_task_merge_task_r_off = _RANDOM[5'hE][23:18];
        mp_grant_task_merge_task_r_alias = _RANDOM[5'hE][25:24];
        mp_grant_task_merge_task_r_vaddr =
          {_RANDOM[5'hE][31:26], _RANDOM[5'hF], _RANDOM[5'h10][5:0]};
        mp_grant_task_merge_task_r_isKeyword = _RANDOM[5'h10][6];
        mp_grant_task_merge_task_r_opcode = _RANDOM[5'h10][10:7];
        mp_grant_task_merge_task_r_param = _RANDOM[5'h10][13:11];
        mp_grant_task_merge_task_r_sourceId = _RANDOM[5'h10][23:17];
        io_msInfo_bits_blockRefill_REG = _RANDOM[5'h15][8];
        io_msInfo_bits_blockRefill_REG_1 = _RANDOM[5'h15][9];
        io_msInfo_bits_blockRefill_REG_2 = _RANDOM[5'h15][10];
        acquire_ts = _RANDOM[5'h15][11];
        probe_ts = _RANDOM[5'h15][12];
        release_ts = _RANDOM[5'h15][13];
      `endif // RANDOMIZE_REG_INIT
      if (reset) begin
        gotT = 1'h0;
        gotDirty = 1'h0;
  ...
  assign acquire_period = 64'(timer - {63'h0, acquire_ts});
  assign probe_period = 64'(timer - {63'h0, probe_ts});
  assign release_period = 64'(timer - {63'h0, release_ts});
endmodule
