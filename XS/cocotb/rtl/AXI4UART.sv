...
module AXI4UART(
    ...
    always @(posedge clock) begin
      if (_nodeIn_b_bits_user_T & ~reset & nodeIn_aw_bits_burst != 2'h1) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: only support busrt ince!\n    at AXI4SlaveModule.scala:86 assert(in.aw.bits.burst === AXI4Parameters.BURST_INCR, \"only support busrt ince!\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (_GEN & nodeIn_ar_bits_burst != 2'h1) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: only support busrt ince!\n    at AXI4SlaveModule.scala:89 assert(in.ar.bits.burst === AXI4Parameters.BURST_INCR, \"only support busrt ince!\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (_GEN
        ...
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at AXI4SlaveModule.scala:141 assert(\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  ...
  wire [7:0]  _GEN_0 = nodeIn_w_bits_strb >> waddr[2:0];
  wire [31:0] _GEN_1 = {{8{_GEN_0[3]}}, {8{_GEN_0[2]}}, {8{_GEN_0[1]}}, {8{_GEN_0[0]}}};
  wire [31:0] _GEN_2 = nodeIn_w_bits_data[31:0] & _GEN_1;
  always @(posedge clock) begin
    if (reset) begin
      state <= 2'h0;
      readBeatCnt <= 8'h0;
      ...
      if (_io_extra_out_valid_T_2 & waddr == 4'h8)
        stat <= _GEN_2 | ~_GEN_1 & stat;
      if (_io_extra_out_valid_T_2 & waddr == 4'hC)
        ctrl <= _GEN_2 | ~_GEN_1 & ctrl;
    end
