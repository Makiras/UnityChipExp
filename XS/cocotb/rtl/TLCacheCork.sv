...
module TLCacheCork(
  ...
  wire         nodeOut_a_valid =
    beatsLeft ? state_0 & winner_0 | state_1 & a_a_valid : _nodeOut_a_valid_T;
  reg          beatsLeft_1;
  ...
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset
          & ~(~auto_in_c_valid | _nodeIn_c_ready_T | (&auto_in_c_bits_opcode))) begin
      ...
      if (~reset
          & ~((~auto_out_d_valid | ~winner_1_1)
              & (~(auto_out_d_valid | winner_1_1) | ~winner_1_2))) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at Arbiter.scala:77 assert((prefixOR zip winner) map { case (p,w) => !p || !w } reduce {_ && _})\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      ...
      if (~reset
          & ~(~(_in_d_valid_T | _q_1_io_deq_valid) | auto_out_d_valid | winner_1_1
              | winner_1_2)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at Arbiter.scala:79 assert (!valids.reduce(_||_) || winner.reduce(_||_))\n");
        if (`STOP_COND_)
  ...
  assign auto_out_a_bits_user_reqSource =
    (muxState_0 ? auto_in_c_bits_user_reqSource : 4'h0)
    | (muxState_1 ? auto_in_a_bits_user_reqSource : 4'h0);
