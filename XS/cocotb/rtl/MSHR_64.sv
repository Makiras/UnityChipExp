...
module MSHR_64(
  ...
  input  [15:0] io_dirResult_bits_idOH,
  ...
  wire            _GEN_1 = req_param == 3'h2;
  wire            _GEN_2 = req_param == 3'h1;
  wire            _GEN_3 = _GEN_1 | meta_self_state[1] & _GEN_2;
  wire            _GEN_4 = meta_self_hit & meta_self_state[0] & _GEN_2;
  wire            _GEN_5 = _probe_clients_T_1 & meta_clients_states_0_hit;
  wire            _GEN_6 = req_opcode != 3'h5;
  wire            _GEN_7 = io_releaseThrough & io_dirResult_valid & req_channel[2];
  wire            will_release_through =
    _GEN_7 & ~((|iam) & meta_clients_states_0_hit) & req_param < 3'h3;
  wire            will_drop_release = _GEN_7 & ~will_release_through;
  wire            _GEN_8 = io_dirResult_valid & req_channel[1];
  wire            will_probeack_through =
    _GEN_8 & (clients_have_T | req_needProbeAckData & (|highest_perm) & ~meta_self_hit);
  wire            no_wait =
    w_probeacklast & w_grantlast & w_releaseack & w_grantack & w_putwritten & w_sinkcack;
  wire            clients_meta_busy =
  ...
  wire [2:0]      io_tasks_source_d_bits_opcode_0 =
    req_channel[0] ? _GEN_9[req_opcode] : 3'h6;
  reg  [3:0]      io_tasks_sink_c_bits_bufIdx_r;
  wire [29:0]     io_tasks_prefetch_resp_bits_tag_0 = {2'h0, req_tag};
  wire            _GEN_10 = io_tasks_source_b_ready & io_tasks_source_b_valid_0;
  ...
  wire            _GEN_11 = req_valid & sinkc_resp_last;
  assign client_probeack_param_vec_0 =
    _GEN_11 ? io_resps_sink_c_bits_param : client_probeack_param_vec_reg_0;
  wire            _GEN_12 = req_valid & io_resps_sink_d_valid;
  wire            _GEN_13 =
  ...
  reg             REG_3;
  `ifndef SYNTHESIS
    wire [6:0] _GEN_15 = io_dirResult_bits_idOH[15:9] | io_dirResult_bits_idOH[7:1];
    wire [2:0] _GEN_16 = _GEN_15[6:4] | _GEN_15[2:0];
    wire       _GEN_17 = client_probeack_param_vec_0 == 3'h3;
    wire       _GEN_18 = ~(req_channel[2]) & ~(req_channel[1]);
    wire       _GEN_19 = io_dirResult_valid & ~_GEN_0 & ~(req_channel[2]);
    wire       _GEN_20 = _GEN_19 & req_channel[1];
    always @(posedge clock) begin
      if (~reset & ~REG) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: Directory result was sent to mismatch MSHR(mshrId:%d, resultId:%d)\n    at MSHR.scala:66 assert(\n",
                 io_id,
                 {|(io_dirResult_bits_idOH[15:8]),
                  |(_GEN_15[6:3]),
                  |(_GEN_16[2:1]),
                  _GEN_16[2] | _GEN_16[0]});
        if (`STOP_COND_)
      ...
      if (~(req_channel[2]) & req_channel[1] & ~reset & _GEN_17) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: ProbeAck toT is not allowed in current design\n    at MSHR.scala:225 assert(param =/= TtoT, \"ProbeAck toT is not allowed in current design\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (_GEN_18 & ~reset & _GEN_17) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: ProbeAck toT is not allowed in current design\n    at MSHR.scala:225 assert(param =/= TtoT, \"ProbeAck toT is not allowed in current design\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      ...
      if (~reset & ~REG_1) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: L3 Release should always hit: mshrId:[%d] addr: [%x]\n    at MSHR.scala:479 assert(RegNext(!meta_valid || !req.fromC || req.fromCmoHelper || clients_meta(iam).hit),\n",
                 io_id, {req_tag, req_set, 6'h0});
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (_GEN_20 & meta_self_hit & ~_GEN_3 & ~reset
          & ~(_io_tasks_source_d_bits_useBypass_T_1 & _GEN_2)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: Probe not supported: [self: %d] [req: %d]\n\n    at MSHR.scala:693 assert(probe_is_report(self_meta.state, req.param),\n",
                 meta_self_state, req_param);
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (_GEN_20 & ~_GEN_4 & meta_clients_states_0_hit & ~reset
          & ~(_GEN_1 | meta_clients_states_0_state[1] & _GEN_2
              | meta_clients_states_0_state == 2'h1 & _GEN_2)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: Invalid Probe Request: [client: %d] [req: %d]\n\n    at MSHR.scala:703 assert(\n",
                 meta_clients_states_0_state, req_param);
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (_GEN_19 & ~(req_channel[1]) & ~_GEN_5 & ~reset
          & ~(_GEN_6 | req_preferCache)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: Hint should always preferCache!\n    at MSHR.scala:848 assert(req.opcode =/= Hint || req.preferCache, \"Hint should always preferCache!\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (_GEN_7 & ~reset & ~req_valid) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at MSHR.scala:892 assert(req_valid)\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (io_dirResult_valid & req_channel[2] & ~reset
          & 2'({1'h0, will_release_through}
               + 2'({1'h0, will_drop_release} + {1'h0, ~_GEN_7})) != 2'h1) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: release ploicy conflict! [through drop save]: [%b %b %b]\n    at MSHR.scala:905 assert(\n",
                 will_release_through, will_drop_release, ~_GEN_7);
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (req_valid & req_channel[0] & req_acquire & client_dir_conflict
          & probe_helper_finish & ~reset & ~REG_2) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: Error L3: meta still conflict when probe helper finish! mshrId: %d\n    at MSHR.scala:954 assert(RegNext(!clients_meta_busy, true.B),\n",
                 io_id);
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (_GEN_10 & ~reset & ~probe_clients) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: Invalid probe task\n\n    at MSHR.scala:1276 assert(io.tasks.source_b.bits.clients =/= 0.U, \"Invalid probe task\\n\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if ((`PRINTF_COND_) & 1'h0)
        $fwrite(32'h80000002, "BUG_REPRODUCE: nested acquire perm alias\n");
  ...
  always @(posedge clock) begin
    ...
    else if (_GEN_12 & _GEN_13 & (|(req_opcode[2:1])))
    ...
    if (io_dirResult_valid) begin
      ...
      client_dir_conflict <= req_channel[0] & req_acquire & clients_meta_busy;
      probe_clients <=
    ...
    REG_1 <=
      ~meta_valid | ~(req_channel[2]) | req_fromCmoHelper | meta_clients_states_0_hit;
  ...
  wire            _GEN_22 = (|req_param) & meta_self_hit;
  wire            _GEN_23 = ~meta_self_hit & req_opcode[0];
  wire            _GEN_24 =
    _GEN_7 | ~(_GEN_23 & (|meta_self_state) & replace_need_release);
  wire            _GEN_25 =
    _GEN_0 | ~(req_channel[2]) | req_fromCmoHelper | ~(req_opcode[0]);
  wire            _GEN_26 = _GEN_4 | ~meta_clients_states_0_hit;
  wire            _GEN_27 =
  ...
  wire            _GEN_28 = req_channel[1] | _GEN_5 | ~_GEN_27;
  wire            _GEN_29 =
    io_dirResult_valid
      ? _GEN_0
        | (req_channel[2]
             ? (req_fromCmoHelper
                  ? ~_GEN_22
                  : ~(will_release_through & req_opcode[0]) & _GEN_24)
             : _GEN_28)
      : w_releaseack;
  wire            _GEN_30 = req_channel[2] | req_channel[1];
  wire            _GEN_31 = _GEN_0 | _GEN_30;
  wire            _GEN_32 =
    req_acquire
      ? (req_needT ? ~(highest_perm_except_me[1]) : highest_perm_except_me == 2'h0)
      : req_needT ? ~(highest_perm[1]) : _prefetch_miss_need_acquire_T_1;
  wire            _GEN_33 = _GEN_31 | _GEN_5 | ~_GEN_32;
  wire            _GEN_34 =
    ~_GEN_32
    | ~(~acquirePermMiss
        & (meta_self_hit & ~_io_tasks_source_a_bits_param_T_1 | preferCache));
  wire            _GEN_35 = req_opcode != 3'h4 & (|(req_opcode[2:1]));
  wire            _GEN_36 =
    meta_clients_states_0_hit
  ...
  wire            _GEN_38 = _GEN_35 ? ~(|iam) | ~_GEN_36 : ~_GEN_37;
  wire            _GEN_39 =
    _GEN_0
    | (req_channel[2]
         ? ~req_fromCmoHelper | ~meta_clients_states_0_hit
         : req_channel[1] ? _GEN_26 : _GEN_5 | _GEN_38);
  wire            _GEN_40 =
    io_dirResult_valid
      ? _GEN_0
        | (req_channel[2]
             ? req_fromCmoHelper | _GEN_7 | ~_GEN_23
             : req_channel[1] | _GEN_5
               | ~(~meta_self_hit & preferCache
                   & (_io_tasks_source_a_bits_param_T_1 | _acquire_param_T_4
                      | prefetch_need_data))
               & (_GEN_35
                    ? ~((|iam) & _GEN_36) | meta_self_hit
                    : ~_GEN_37 | meta_self_hit))
      : s_wbselftag;
  wire            _GEN_41 = io_tasks_source_a_ready & io_tasks_source_a_valid_0;
  wire            _GEN_42 = io_tasks_source_c_ready & io_tasks_source_c_valid_0;
  wire            _GEN_43 = io_tasks_tag_write_ready & io_tasks_tag_write_valid_0;
  wire            _GEN_44 = io_tasks_sink_c_ready & io_tasks_sink_c_valid_0;
  wire            _GEN_45 = _GEN_11 & sink_c_resp_valid;
  wire            _GEN_46 = someClientHasProbeAckData | io_resps_sink_c_bits_hasData;
  wire            _GEN_47 = _GEN_45 & _GEN_46;
  wire            _GEN_48 =
    req_channel[1]
    & (probeAckDataThrough & io_resps_sink_c_bits_param != 3'h5 | ~req_fromProbeHelper);
  wire            _GEN_49 = req_valid & sink_c_resp_valid;
  wire            _GEN_50 =
  ...
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      REG <= ~io_dirResult_valid | req_valid & ~meta_valid;
      promoteT_safe <= ~_GEN_51 & (io_dirResult_valid | promoteT_safe);
      if (_GEN_14) begin
        ...
        w_grantlast <= ~_GEN_51 & (io_dirResult_valid ? _GEN_33 : w_grantlast);
        w_grant <= ~_GEN_51 & (io_dirResult_valid ? _GEN_33 : w_grant);
      end
      if (io_dirResult_valid) begin
        a_do_release <= ~_GEN_31 & ~_GEN_5 & _GEN_27;
        a_do_probe <= ~_GEN_31 & ~_GEN_5 & (_GEN_35 ? (|iam) & _GEN_36 : _GEN_37);
      ...
      if (_GEN_49) begin
        ...
        if (io_dirResult_valid) begin
          w_probeackfirst <= _GEN_39;
          w_probeacklast <= _GEN_39;
          w_probeack <= _GEN_39;
        end
      end
      inv_self_dir <= _GEN_49 & _GEN_52 & ~_GEN_53 | ~io_dirResult_valid & inv_self_dir;
      if (_GEN_11)
        client_probeack_param_vec_reg_0 <= io_resps_sink_c_bits_param;
      releaseThrough <= ~will_be_free & (_GEN_7 ? will_release_through : releaseThrough);
      releaseDrop <= ~will_be_free & (_GEN_7 ? will_drop_release : releaseDrop);
      probeAckDataThrough <=
        ~will_be_free
        & (io_dirResult_valid
             ? (req_fromCmoHelper
      ...
      probeAckDataDrop <=
        ...
        & (io_dirResult_valid
             ? (req_fromCmoHelper
                  ? ~(|req_param)
                  : req_channel[1] ? _GEN_8 & ~will_probeack_through : probeAckDataDrop)
             : probeAckDataDrop);
      probeAckDataSave <=
        ~will_be_free
        & (io_dirResult_valid
                  ...
                  : req_channel[1]
                      ? (req_fromProbeHelper
                           ? will_probeack_through & ~io_probeAckDataThrough
                             & meta_self_hit
                           : ~_GEN_8 | will_probeack_through & ~io_probeAckDataThrough
                             & meta_self_hit & req_param == 3'h1)
                      : ~probeAckDataThrough & ~probeAckDataDrop)
             : probeAckDataSave);
      someClientHasProbeAckData <=
        ~will_be_free
        & (_GEN_11 & io_resps_sink_c_bits_hasData | someClientHasProbeAckData);
      nested_c_hit_reg <= ~io_dirResult_valid & nested_c_hit;
      s_acquire <= ~_GEN_51 & (_GEN_41 | (io_dirResult_valid ? _GEN_33 : s_acquire));
      s_probe <= _GEN_10 | (io_dirResult_valid ? _GEN_39 : s_probe);
      s_release <=
        _GEN_42
        | (io_dirResult_valid
             ? _GEN_0
               | (req_channel[2] ? (req_fromCmoHelper ? ~_GEN_22 : _GEN_24) : _GEN_28)
             : s_release);
      s_probeack <=
        (~_GEN_45 | _GEN_46 | ~_GEN_48)
        & (_GEN_42
           | (io_dirResult_valid
                ? (_GEN_0
                     ? ~(req_channel[1])
      ...
      s_execute <=
        ...
        | (io_dirResult_valid
             ? (_GEN_0
                  ...
                  : req_channel[2]
                      ? req_fromCmoHelper
                      : req_channel[1] | _GEN_5 | _probe_clients_T_1)
             : s_execute);
      s_grantack <=
        ~(_GEN_49 & _GEN_50 & ~bypassGet & ~bypassPut_latch_reg)
        & (io_tasks_source_e_ready & io_tasks_source_e_valid_0
           | (io_dirResult_valid
                ? _GEN_31 | ~(~_GEN_5 & _GEN_32) | ~(~bypassGet & ~bypassPut)
                : s_grantack));
      s_wbselfdir <=
        ...
        | (io_dirResult_valid
             ? _GEN_0
               | (req_channel[2]
                    ? (req_fromCmoHelper
                         ? ~meta_self_hit
                         : _GEN_7 | ~(meta_self_hit | req_opcode[0]))
                    : req_channel[1]
                        ? ~meta_self_hit | ~_GEN_3
                        : _GEN_5
                          | ~(~(|(req_opcode[2:1])) & meta_self_hit & ~meta_self_dirty
                              | req_acquire & ~acquirePermMiss
                              & (meta_self_hit | preferCache))
                          & (_GEN_35
                               ? ~((|iam) & _GEN_36 & ~meta_self_hit) & _GEN_34
                               : ~_GEN_37 & _GEN_34))
             : s_wbselfdir);
      if (_GEN_49 & _GEN_52)
        s_wbselftag <= ~_GEN_53 | _GEN_43 | _GEN_40;
      else
        s_wbselftag <= _GEN_43 | _GEN_40;
      s_wbclientsdir <=
        ...
        | (io_dirResult_valid
             ? _GEN_0
               | (req_channel[2]
                    ...
                    : req_channel[1] ? _GEN_26 : _GEN_5 | ~req_acquire & _GEN_38)
             : s_wbclientsdir);
      s_wbclientstag <=
        io_tasks_client_tag_write_ready & io_tasks_client_tag_write_valid_0
        | (io_dirResult_valid
             ? _GEN_31 | ~(~_GEN_5 & req_acquire) | meta_clients_states_0_hit
             : s_wbclientstag);
      s_transferput <=
        _GEN_41 | (io_dirResult_valid ? _GEN_31 | _GEN_5 | ~bypassPut : s_transferput);
      s_writerelease <=
        _GEN_44 & s_writeprobe | (io_dirResult_valid ? _GEN_25 : s_writerelease);
      s_writeprobe <= ~_GEN_47 & (_GEN_44 | io_dirResult_valid | s_writeprobe);
      s_triggerprefetch <=
        io_tasks_prefetch_train_ready & ~s_triggerprefetch
        | (io_dirResult_valid
             ? _GEN_31 | _GEN_5
               | ~(_GEN_6 & req_needHint & (~meta_self_hit | meta_self_prefetch))
             : s_triggerprefetch);
      s_prefetchack <=
        io_tasks_prefetch_resp_ready & io_tasks_prefetch_resp_valid_0
        | (io_dirResult_valid
             ? _GEN_31 | _GEN_5 | ~(_probe_clients_T_1 & req_isBop)
             : s_prefetchack);
      w_grantfirst <= _GEN_14 | ~_GEN_51 & (io_dirResult_valid ? _GEN_33 : w_grantfirst);
      w_releaseack <=
        _GEN_12 & io_resps_sink_d_bits_opcode == 3'h6
        | (_GEN_45
             ? (_GEN_46
                  ? ~(req_channel[1] & req_fromProbeHelper & probeAckDataThrough
                      | req_fromCmoHelper & probeAckDataThrough) & _GEN_29
                  : ~(_GEN_48 & req_fromProbeHelper) & _GEN_29)
             : _GEN_29);
      w_grantack <=
        ...
        | (io_dirResult_valid
             ? (_GEN_0
                  ...
                  : _GEN_30 | _GEN_5 | ~req_acquire)
             : w_grantack);
      w_putwritten <=
        io_resps_source_d_valid
        | (io_dirResult_valid
             ? _GEN_31 | _GEN_5 | ~(~(|(req_opcode[2:1])) & ~bypassPut)
             : w_putwritten);
      w_sinkcack <=
        io_resps_sink_c_ack_valid | ~_GEN_47
        & (io_dirResult_valid ? _GEN_25 : w_sinkcack);
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        req_reqSource = _RANDOM[4'h3][21:18];
        req_valid = _RANDOM[4'h3][22];
        iam = _RANDOM[4'h3][24:23];
        meta_reg_self_dirty = _RANDOM[4'h4][9];
        meta_reg_self_state = _RANDOM[4'h4][11:10];
        meta_reg_self_clientStates_0 = _RANDOM[4'h4][13:12];
        meta_reg_self_prefetch = _RANDOM[4'h4][14];
        meta_reg_self_hit = _RANDOM[4'h4][15];
        meta_reg_self_way = _RANDOM[4'h4][19:16];
        meta_reg_self_tag = {_RANDOM[4'h4][31:20], _RANDOM[4'h5][15:0]};
        meta_reg_self_error = _RANDOM[4'h5][16];
        meta_reg_clients_states_0_state = _RANDOM[4'h5][18:17];
        meta_reg_clients_states_0_hit = _RANDOM[4'h5][19];
        meta_reg_clients_tag = {_RANDOM[4'h5][31:21], _RANDOM[4'h6][18:0]};
        meta_reg_clients_way = _RANDOM[4'h6][22:19];
        meta_reg_clients_error = _RANDOM[4'h6][23];
        meta_valid = _RANDOM[4'h7][25];
        REG = _RANDOM[4'h7][26];
        promoteT_safe = _RANDOM[4'h7][27];
        gotT = _RANDOM[4'h7][28];
        a_do_release = _RANDOM[4'h7][30];
        a_do_probe = _RANDOM[4'h7][31];
        probe_dirty = _RANDOM[4'h8][0];
        highest_perm_reg_reg = _RANDOM[4'h8][3:2];
        inv_self_dir = _RANDOM[4'h8][5];
        client_probeack_param_vec_reg_0 = _RANDOM[4'h8][8:6];
        transmit_from_other_client_reg = _RANDOM[4'h8][9];
        releaseThrough = _RANDOM[4'h8][10];
        releaseDrop = _RANDOM[4'h8][11];
        probeAckDataThrough = _RANDOM[4'h8][12];
        probeAckDataDrop = _RANDOM[4'h8][13];
        probeAckDataSave = _RANDOM[4'h8][14];
        someClientHasProbeAckData = _RANDOM[4'h8][15];
        nested_c_hit_reg = _RANDOM[4'h8][16];
        sink = _RANDOM[4'h8][19:17];
        bad_grant = _RANDOM[4'h8][20];
        REG_1 = _RANDOM[4'h8][21];
        s_acquire = _RANDOM[4'h8][22];
        s_probe = _RANDOM[4'h8][23];
        s_release = _RANDOM[4'h8][24];
        s_probeack = _RANDOM[4'h8][25];
        s_execute = _RANDOM[4'h8][26];
        s_grantack = _RANDOM[4'h8][27];
        s_wbselfdir = _RANDOM[4'h8][28];
        s_wbselftag = _RANDOM[4'h8][29];
        s_wbclientsdir = _RANDOM[4'h8][30];
        s_wbclientstag = _RANDOM[4'h8][31];
        s_transferput = _RANDOM[4'h9][0];
        s_writerelease = _RANDOM[4'h9][1];
        s_writeprobe = _RANDOM[4'h9][2];
        s_triggerprefetch = _RANDOM[4'h9][3];
        s_prefetchack = _RANDOM[4'h9][4];
        w_probeackfirst = _RANDOM[4'h9][5];
        w_probeacklast = _RANDOM[4'h9][6];
        w_probeack = _RANDOM[4'h9][7];
        w_grantfirst = _RANDOM[4'h9][8];
        w_grantlast = _RANDOM[4'h9][9];
        w_grant = _RANDOM[4'h9][10];
        w_releaseack = _RANDOM[4'h9][11];
        w_grantack = _RANDOM[4'h9][12];
        w_putwritten = _RANDOM[4'h9][13];
        w_sinkcack = _RANDOM[4'h9][14];
        acquire_flag = _RANDOM[4'h9][15];
        nestC_save_flag = _RANDOM[4'h9][16];
        nestC_saveDirty_flag = _RANDOM[4'h9][17];
        bypassPut_latch_reg = _RANDOM[4'h9][18];
        client_dir_conflict = _RANDOM[4'h9][19];
        probe_helper_finish = _RANDOM[4'h9][20];
        REG_2 = _RANDOM[4'h9][21];
        probe_clients = _RANDOM[4'h9][22];
        io_tasks_sink_c_bits_bufIdx_r = _RANDOM[4'h9][26:23];
        no_schedule_REG = _RANDOM[4'h9][27];
        REG_3 = _RANDOM[4'h9][28];
        preferCache_latch = _RANDOM[4'h9][30];
        transmit_from_other_client_latch = _RANDOM[4'h9][31];
        a_c_through_reg = _RANDOM[4'hA][0];
      `endif // RANDOMIZE_REG_INIT
      if (reset) begin
        req_valid = 1'h0;
        meta_valid = 1'h0;
  ...
  assign io_status_bits_is_miss = ~meta_self_hit;
  ...
  assign io_status_bits_is_prefetch = req_isPrefetch;
  assign io_ecc_valid =
