...
module SourceD(
  ...
  wire [2:0]   _pipe_io_out_bits_req_channel;
  ...
  wire         _io_d_valid_T = winner_0 | s2_d_valid;
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset & ~(~s1_bypass_hit | _s1_queue_io_enq_ready)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at SourceD.scala:91 assert(!s1_queue.io.enq.valid || s1_queue.io.enq.ready)\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset
          & ~(~s3_valid | _pipe_io_out_bits_req_channel[0]
              & (_pipe_io_out_bits_req_opcode == 3'h5
                 | _pipe_io_out_bits_req_opcode == 3'h1
                 | _pipe_io_out_bits_req_opcode == 3'h0
                 & ~_pipe_io_out_bits_req_bypassPut))) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: Only data task can go to stage3!\n    at SourceD.scala:210 assert(!s3_valid || needData(s3_regs.req), \"Only data task can go to stage3!\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & ~(~s3_queue_io_enq_valid_REG_4 | _s3_queue_io_enq_ready)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at SourceD.scala:237 assert(!s3_queue.io.enq.valid || s3_queue.io.enq.ready)\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      ...
      if (~reset & ~(~_io_d_valid_T | winner_0 | winner_1)) begin
        if (`ASSERT_VERBOSE_COND_)
  ...
  wire         io_d_valid_0 =
    beatsLeft ? state_0 & winner_0 | state_1 & s2_d_valid : _io_d_valid_T;
