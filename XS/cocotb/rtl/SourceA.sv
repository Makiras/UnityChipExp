...
module SourceA(
  ...
  output [3:0]   io_a_bits_user_reqSource,
  ...
  wire         _io_a_valid_T = s1_full | a_acquire_valid;
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset & ~(~s1_full | ~winner_1)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at Arbiter.scala:77 assert((prefixOR zip winner) map { case (p,w) => !p || !w } reduce {_ && _})\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & ~(~_io_a_valid_T | s1_full | winner_1)) begin
        if (`ASSERT_VERBOSE_COND_)
  ...
  wire         io_a_valid_0 =
    beatsLeft ? state_0 & s1_full | state_1 & a_acquire_valid : _io_a_valid_T;
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        s0_count = _RANDOM[4'h3][7];
        s1_task_tag = {_RANDOM[4'h3][31:9], _RANDOM[4'h4][4:0]};
        s1_task_set = _RANDOM[4'h4][16:5];
        s1_task_off = _RANDOM[4'h4][22:17];
        s1_task_opcode = _RANDOM[4'h5][25:23];
        s1_task_param = _RANDOM[4'h5][28:26];
        s1_task_source = {_RANDOM[4'h5][31:29], _RANDOM[4'h6][0]};
        s1_task_size = _RANDOM[4'h6][7:5];
        s1_pb_latch_REG = _RANDOM[4'h6][14];
        s1_pb_latch_r_data =
          {_RANDOM[4'h6][31:15],
           _RANDOM[4'h7],
           _RANDOM[4'h8],
           ...
           _RANDOM[4'hD],
           _RANDOM[4'hE][14:0]};
        s1_pb_latch_r_mask = {_RANDOM[4'hE][31:15], _RANDOM[4'hF][14:0]};
        beatsLeft = _RANDOM[4'hF][16];
        state_0 = _RANDOM[4'hF][17];
        state_1 = _RANDOM[4'hF][18];
      `endif // RANDOMIZE_REG_INIT
      if (reset) begin
        busy = 1'h0;
        s1_full = 1'h0;
  ...
  assign io_a_bits_user_reqSource = muxState_1 ? io_task_bits_reqSource : 4'h0;
