...
module RefillBuffer(
  ...
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (io_r_valid & ~io_r_beat & ~reset & ~io_r_ready_0) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: [%d] first beat must hit!\n    at RefillBuffer.scala:48 assert(r.ready, \"[%%%%d] first beat must hit!\", r.id)\n",
                 io_r_id);
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (buffer_MPORT_en & ~reset
          & (io_w_beat ? _GEN_0[io_w_id_0[2:0]] : _GEN[io_w_id_0[2:0]])) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: [%d] attempt to write a valid entry\n    at RefillBuffer.scala:66 assert(!valids(w.id)(w.beat), \"[%%%%d] attempt to write a valid entry\", w.id)\n",
                 io_w_id_0);
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
