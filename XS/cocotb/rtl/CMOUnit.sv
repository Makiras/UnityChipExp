    ...
    always @(posedge clock) begin
      if (~reset & (|state) & io_req_valid) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at MissQueue.scala:329 assert(!(state =/= s_idle && io.req.valid))\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & state != 2'h2 & io_resp_chanD_valid) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at MissQueue.scala:330 assert(!(state =/= s_wresp && io.resp_chanD.valid))\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  ...
  always @(posedge clock or posedge reset) begin
    if (reset)
      ...
      if (state == 2'h1) begin
        ...
        if (io_resp_chanD_ready_0 & io_resp_chanD_valid)
          state <= 2'h3;
      end
