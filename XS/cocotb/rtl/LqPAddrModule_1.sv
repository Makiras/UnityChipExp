    ...
    always @(posedge clock) begin
      if (~reset & io_wen_0 & io_wen_1 & io_waddr_0 == io_waddr_1) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at LoadQueueData.scala:118 assert(!(io.wen(i) && io.wen(j) && io.waddr(i) === io.waddr(j)))\n");
        if (`STOP_COND_)
