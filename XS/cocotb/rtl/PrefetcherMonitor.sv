...
module PrefetcherMonitor(
  ...
  wire        trigger_disable_probe = validity_reset & bad_prefetch_cnt > 11'h383;
  wire        timely_reset =
  ...
  wire        _GEN =
    timely_reset & late_hit_prefetch_cnt > 11'h383 | trigger_disable_probe;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (timely_reset & late_miss_prefetch_cnt > 11'hC7) begin
        enable <= ~trigger_disable_probe & _enable_T;
        confidence <= ~trigger_disable_probe & _confidence_T;
      end
      else begin
