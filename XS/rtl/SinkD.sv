...
module SinkD(
  ...
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset & ~(~io_d_valid | ~(io_d_bits_opcode[0]) | io_d_bits_size == 3'h6)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: SinkD must receive aligned message when needData\n    at SinkD.scala:55 assert(!io.d.valid || !needData || io.d.bits.size === log2Up(blockBytes).U, \"SinkD must receive aligned message when needData\")\n");
        if (`STOP_COND_)
