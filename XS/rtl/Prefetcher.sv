...
module Prefetcher(
  ...
  input         io_resp_valid,
  input  [3:0]  io_resp_bits_pfSource,
  ...
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset
          & ~(~_pfRcv_io_req_valid | _pfRcv_io_req_bits_pfSource == 4'hA
              | _pfRcv_io_req_bits_pfSource == 4'hB
              | _pfRcv_io_req_bits_pfSource == 4'hC)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at Prefetcher.scala:322 assert(!pfRcv.get.io.req.valid ||\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  `endif // not def SYNTHESIS
  wire        _pftQueue_io_enq_valid_T_probe = _vbop_io_req_valid | _pbop_io_req_valid;
  wire        _pftQueue_io_enq_bits_T = _pfRcv_io_req_valid | _vbop_io_req_valid;
  wire        _GEN = _vbop_io_req_valid & ~_pfRcv_io_req_valid;
  wire        _GEN_0 = _pbop_io_req_valid & ~_pfRcv_io_req_valid & ~_vbop_io_req_valid;
  wire        _GEN_1 = (_pbop_io_req_valid | _vbop_io_req_valid) & ~_pfRcv_io_req_valid;
  wire        _GEN_2 =
    _tp_io_req_valid & ~_pfRcv_io_req_valid & ~_vbop_io_req_valid & ~_pbop_io_req_valid;
  wire        _GEN_3 =
    _pfRcv_io_req_valid & (_pftQueue_io_enq_valid_T_probe | _tp_io_req_valid);
  ...
  PBestOffsetPrefetch pbop (
    ...
    .io_resp_valid        (io_resp_valid & io_resp_bits_pfSource == 4'h9)
  );
  wire        _probe;
  assign _probe = _pbop_io_req_valid;
  VBestOffsetPrefetch vbop (
    ...
    .io_tlb_req_resp_bits_excp_0_gpf_st (io_tlb_req_resp_bits_excp_0_gpf_st),
    ...
    .io_resp_valid                      (io_resp_valid & io_resp_bits_pfSource == 4'h8)
  );
  wire        _probe_0;
  assign _probe_0 = _vbop_io_req_valid;
  TemporalPrefetch tp (
    .clock                              (clock),
    .reset                              (reset),
  ...
  wire        _probe_1;
  assign _probe_1 = _tp_io_req_valid;
  ...
  wire        _probe_2;
  assign _probe_2 = _pfRcv_io_req_valid;
  PrefetchQueue pftQueue (
    .clock                (clock),
    .reset                (reset),
    .io_enq_valid
      (_pfRcv_io_req_valid | l2_pf_en
       & (_pftQueue_io_enq_valid_T_probe | _tp_io_req_valid)),
    .io_enq_bits_tag
