...
module CustomL1Hint(
  ...
  wire       _hint_s1Queue_io_enq_ready;
  ...
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset & ~_hint_s1Queue_io_enq_ready) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: hint_s1Queue should never be full\n    at CustomL1Hint.scala:110 assert(hint_s1Queue.io.enq.ready, \"hint_s1Queue should never be full\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & ~_hintQueue_io_enq_ready) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: hintQueue should never be full\n    at CustomL1Hint.scala:111 assert(hintQueue.io.enq.ready, \"hintQueue should never be full\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
