...
module ICacheMissUnit(
  ...
  wire              _fetchDemux_io_in_valid_T_1 = io_fetch_req_valid & ~fetchHit;
  ...
  wire              _priorityFIFO_io_enq_valid_T_probe =
    _prefetchDemux_io_in_ready & _prefetchDemux_io_in_valid_T_1;
  ...
  wire [12:0]       _refill_done_r_beats1_decode_T = 13'h3F << io_mem_grant_bits_size;
  wire              refill_done_r_beats1 =
    io_mem_grant_bits_opcode[0] & ~(_refill_done_r_beats1_decode_T[5]);
    ...
    always @(posedge clock) begin
      if (~reset
        ...
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: priorityFIFO.io.enq and io.prefetch_req must fire at the same cycle\n    at ICacheMissUnit.scala:311 assert(\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset
          & (_priorityFIFO_io_deq_ready_T & _priorityFIFO_io_deq_valid
             ^ _priorityFIFO_io_deq_ready_T)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: priorityFIFO.io.deq and prefetchArb.io.out must fire at the same cycle\n    at ICacheMissUnit.scala:315 assert(\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset
          & ((refill_done_r_counter | ~refill_done_r_beats1) & io_mem_grant_valid
             ^ last_fire)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: refill not done!\n    at ICacheMissUnit.scala:339 assert(!(refill_done ^ last_fire), \"refill not done!\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  ...
  wire              _GEN_3 = _fetchDemux_io_in_ready & _fetchDemux_io_in_valid_T_1;
  wire              _GEN_4 = io_mem_grant_valid & io_mem_grant_bits_opcode[0];
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      corrupt_r <= _GEN_4 & io_mem_grant_bits_corrupt | ~fetch_resp_valid & corrupt_r;
    end
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        respDataReg_1 =
          {_RANDOM[5'h8][31:1],
           ...
           _RANDOM[5'hF],
           _RANDOM[5'h10][0]};
        refill_done_r_counter = _RANDOM[5'h10][1];
  ...
  DeMultiplexer fetchDemux (
    ...
    .io_in_valid            (_fetchDemux_io_in_valid_T_1),
  ...
  FIFOReg priorityFIFO (
    ...
    .io_deq_ready (_priorityFIFO_io_deq_ready_T),
  ...
  DummyDPICWrapper difftest_module (
    ...
    .io_bits_addr   ({16'h0, io_fetch_resp_bits_blkPaddr_0, 6'h0}),
    .io_bits_data_0 (respDataReg_0[63:0]),
    .io_bits_data_1 (respDataReg_0[127:64]),
    .io_bits_data_2 (respDataReg_0[191:128]),
    .io_bits_data_3 (respDataReg_0[255:192]),
    .io_bits_data_4 (respDataReg_1[63:0]),
    .io_bits_data_5 (respDataReg_1[127:64]),
    .io_bits_data_6 (respDataReg_1[191:128]),
    .io_bits_data_7 (respDataReg_1[255:192]),
    .io_bits_coreid ({7'h0, io_hartId}),
    .io_bits_index  (8'h0)
  );
  ...
  assign io_meta_write_valid = difftest_valid;
  ...
  assign io_data_write_valid = difftest_valid;
