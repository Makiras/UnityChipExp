...
module TemporalPrefetch(
  ...
  wire             _tpDataQueue_io_enq_ready;
  ...
  wire             _dataWriteQueue_io_enq_ready;
  ...
  wire             _GEN_1 = dorecord_s2 & _GEN_0;
  reg              REG;
  ...
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset & ~_tpDataQueue_io_enq_ready) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at TemporalPrefetch.scala:258 assert(tpDataQueue.io.enq.ready === true.B) // tpDataQueue is never full\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (_GEN_1 & ~reset & ~_triggerQueue_io_deq_valid) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at TemporalPrefetch.scala:280 assert(triggerQueue.io.deq.valid)\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & ~(REG | ~write_record)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: tpTable_w_valid can only be true in s3\n    at TemporalPrefetch.scala:288 assert(RegNext(s2_valid, false.B) || !tpTable_w_valid, \"tpTable_w_valid can only be true in s3\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & ~_dataWriteQueue_io_enq_ready) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at TemporalPrefetch.scala:313 assert(dataWriteQueue.io.enq.ready === true.B) // TODO: support back-pressure\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  ...
  wire [3:0][47:0] _GEN_2 =
    {{sending_data_0}, {sending_data_2}, {sending_data_1}, {sending_data_0}};
  wire [47:0]      _GEN_3 = _GEN_2[sending_idx[1:0]];
  wire             _tpDataQueue_io_deq_ready_T_1 = tpDataQFull | ~do_sending;
  wire             _GEN_4 = _tpDataQueue_io_deq_ready_T_1 & _tpDataQueue_io_deq_valid;
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        train_s2_tag = {_RANDOM[5'h4][31:13], _RANDOM[5'h5][13:0]};
        train_s2_set = _RANDOM[5'h5][22:14];
  ...
  Queue4_TPmetaReq dataWriteQueue (
    .clock                 (clock),
    .reset                 (reset),
    .io_enq_ready          (_dataWriteQueue_io_enq_ready),
  ...
  Queue9_tpDataEntry tpDataQueue (
    .clock                 (clock),
    .reset                 (reset),
    .io_enq_ready          (_tpDataQueue_io_enq_ready),
    .io_enq_valid
  ...
  assign io_req_bits_tag = _GEN_3[41:9];
  assign io_req_bits_set = _GEN_3[8:0];
