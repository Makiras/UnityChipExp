...
module MultiWakeupQueue_4(
  ...
  `ifndef SYNTHESIS
    wire [2:0] _GEN =
      3'({1'h0, 2'({1'h0, pipesValidVec_0} + {1'h0, pipesValidVec_1})}
         + {1'h0, 2'({1'h0, _pipes_2_io_deq_valid} + {1'h0, pipesValidVec_3})});
    always @(posedge clock) begin
      if (~reset & (|(_GEN[2:1]))) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: PopCount(pipesValidVec) should be no more than 1\n    at MultiWakeupQueue.scala:73 assert(PopCount(pipesValidVec) <= 1.U, \"PopCount(pipesValidVec) should be no more than 1\")\n");
        if (`STOP_COND_)
