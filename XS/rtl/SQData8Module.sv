    ...
    always @(posedge clock) begin
      if (~reset & io_data_wen_0 & io_data_wen_1
        ...
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at StoreQueueData.scala:211 assert(!(io.data.wen(i) && io.data.wen(j) && io.data.waddr(i) === io.data.waddr(j)))\n");
        if (`STOP_COND_)
      ...
      if (~reset & io_mask_wen_0 & io_mask_wen_1
        ...
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed\n    at StoreQueueData.scala:216 assert(!(io.mask.wen(i) && io.mask.wen(j) && io.mask.waddr(i) === io.mask.waddr(j)))\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
