...
module IPrefetchPipe(
  ...
  `ifndef SYNTHESIS
    wire [2:0] _GEN_2 =
      3'({1'h0, 2'({1'h0, new_info_1[0]} + {1'h0, new_info_1[1]})}
         + {1'h0, 2'({1'h0, new_info_1[2]} + {1'h0, new_info_1[3]})});
    wire [2:0] _GEN_3 =
      3'({1'h0, 2'({1'h0, new_info_1_1[0]} + {1'h0, new_info_1_1[1]})}
         + {1'h0, 2'({1'h0, new_info_1_1[2]} + {1'h0, new_info_1_1[3]})});
    wire [1:0] _s1_itlb_exception_x9_T_2 =
      2'({1'h0, io_itlb_0_resp_bits_excp_0_pf_instr}
         + 2'({1'h0, io_itlb_0_resp_bits_excp_0_gpf_instr}
              + {1'h0, io_itlb_0_resp_bits_excp_0_af_instr}));
    wire [1:0] _s1_itlb_exception_x9_T_12 =
      2'({1'h0, io_itlb_1_resp_bits_excp_0_pf_instr}
         + 2'({1'h0, io_itlb_1_resp_bits_excp_0_gpf_instr}
              + {1'h0, io_itlb_1_resp_bits_excp_0_af_instr}));
    always @(posedge clock) begin
      if (~reset & _s1_itlb_exception_x9_T_2[1]) begin
      ...
      if (~reset & _s1_itlb_exception_x9_T_12[1]) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: ExceptionType.fromOH receives input that is not one-hot: pf=%d, gpf=%d, af=%d\n    at FrontendBundle.scala:122 assert(\n",
                 io_itlb_1_resp_bits_excp_0_pf_instr,
                 io_itlb_1_resp_bits_excp_0_gpf_instr,
      ...
      if (_GEN_1 & ~reset & ~(_GEN_2 < 3'h2 & (_GEN_3 < 3'h2 | ~s1_doubleline))) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: Multi-hit:\nport0: count=%d ptag=0x%x vSet=0x%x vaddr=0x%x\nport1: count=%d ptag=0x%x vSet=0x%x vaddr=0x%x\n    at IPrefetch.scala:368 assert(\n",
                 |(_GEN_2[2:1]), s1_req_paddr_0[47:12], s1_req_vaddr_0[13:6],
                 s1_req_vaddr_0, (|(_GEN_3[2:1])) & s1_doubleline, s1_req_paddr_1[47:12],
                 s1_req_vaddr_1[13:6], s1_req_vaddr_1);
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  ...
  wire        _GEN_4 = s0_can_go & io_req_valid;
  wire        _GEN_5 = _GEN_4 & ~io_req_bits_isSoftPrefetch;
  wire        _GEN_6 = _GEN_4 & io_req_bits_isSoftPrefetch;
  wire        _GEN_7 = io_MSHRReq_ready & _toMSHRArbiter_io_out_valid;
  wire        _GEN_8 = _GEN_7 & ~s2_isSoftPrefetch;
  wire        _GEN_9 = _GEN_7 & s2_isSoftPrefetch;
  wire        _GEN_10 = _toMSHRArbiter_io_out_valid & ~io_MSHRReq_ready;
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        s2_isSoftPrefetch = _RANDOM[5'hE][10];
