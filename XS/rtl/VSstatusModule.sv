...
module VSstatusModule(
  ...
  wire       _GEN_0 = (robCommit_vsDirty | writeVCSR) & isVirtMode;
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (_GEN & ~reset & reg_FS == 2'h0) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: The vsstatus.FS should not be Off when set dirty, please check decode\n    at VirtualSupervisorLevel.scala:30 assert(reg.FS =/= ContextStatus.Off, \"The vsstatus.FS should not be Off when set dirty, please check decode\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (_GEN_0 & ~reset & reg_VS == 2'h0) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: The vsstatus.VS should not be Off when set dirty, please check decode\n    at VirtualSupervisorLevel.scala:35 assert(reg.VS =/= ContextStatus.Off, \"The vsstatus.VS should not be Off when set dirty, please check decode\")\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
