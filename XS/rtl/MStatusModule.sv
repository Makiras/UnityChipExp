  ...
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (~menvcfg_DTE & wAliasSstatus_wen
          | ~(w_wen | trapToHS_mstatus_valid | retFromM_mstatus_valid
              | retFromS_mstatus_valid | wAliasSstatus_wen)) begin
      end
      else
        reg_SDT <=
          trapToHS_mstatus_valid & trapToHS_mstatus_bits_SDT | retFromM_mstatus_valid
          & retFromM_mstatus_bits_SDT | retFromS_mstatus_valid & retFromS_mstatus_bits_SDT
          | wAliasSstatus_wen & wAliasSstatus_wdata[24] | w_wen & w_wdata[24];
      if (w_wen | trapToM_mstatus_valid)
        reg_GVA <= trapToM_mstatus_valid & trapToM_mstatus_bits_GVA | w_wen & w_wdata[38];
      if (w_wen | _GEN_1 | retFromS_mstatus_valid)
        reg_MDT <=
          trapToM_mstatus_valid & trapToM_mstatus_bits_MDT | retFromM_mstatus_valid
          & retFromM_mstatus_bits_MDT | retFromS_mstatus_valid & retFromS_mstatus_bits_MDT
          | w_wen & w_wdata[42];
    end
