...
#define CONFIG_DIFF_STORE_WIDTH 48
typedef struct  {
  uint8_t  valid;
  uint64_t addr;
  uint64_t data;
...
#define CONFIG_DIFFTEST_AIAXTOPEIEVENT
typedef struct  {
  uint8_t  valid;
  uint64_t mtopei;
  uint64_t stopei;
  uint64_t vstopei;
} DifftestAIAXtopeiEvent;
...
typedef struct {
  ...
  DifftestAIAXtopeiEvent         aia_xtopei;
